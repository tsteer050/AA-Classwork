...
def price_like_shiny_mouse
  ...
  execute(<<-SQL) 
    ...
    FROM
      toys
    JOIN toys AS shinytoys
    ON
      toys.price = shinytoys.price
    WHERE
      shinytoys.name = 'Shiny Mouse'
      AND
...
def price_like_shiny_mouse_sub
  ...
  FROM
    toys
  WHERE toys.price IN (SELECT 
                        toys.price
                        FROM
                        toys
                        WHERE
                        toys.name = 'Shiny Mouse') AND 
    toys.name != 'Shiny Mouse'
    ORDER BY toys.name ASC
...
def just_like_orange
  ...
  JOIN
    cats AS orangebreed
    ON
    cats.breed = orangebreed.breed
  WHERE 
    orangebreed.name = 'Orange'
    AND
    cats.name != 'Orange'
...
def just_like_orange_sub
  ...
  FROM 
    cats
  WHERE cats.breed IN (SELECT cats.breed
                      FROM cats
...
def toys_that_jet_owns
  ...
  # DO NOT USE A SUBQUERY cats.name, toys.name
  execute(<<-SQL)
  ...
  JOIN 
    cattoys AS jettoysids
    ON cattoys.toy_id = jettoysids.toy_id
  JOIN 
    cats AS jettoys
    ON jettoys.id = jettoysids.cat_id
  WHERE jettoys.name = 'Jet' AND
        cats.name != 'Jet'
  ORDER BY cats.name ASC
  SQL
end
